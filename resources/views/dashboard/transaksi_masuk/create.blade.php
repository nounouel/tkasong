        ...
        <form action="{{ route('transaksi-masuk.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="mb-5">
                    ...
                    <input id="tanggal" type="date" name="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}" class="form-input" required />
                    @error('tanggal')
                        <span class="text-danger text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    <label for="jumlah" class="dark:text-white-light font-semibold">Jumlah Masuk</label>
                    <input id="jumlah" type="number" name="jumlah" value="{{ old('jumlah') }}" min="1" class="form-input" placeholder="Masukkan jumlah barang masuk" required />
                    @error('jumlah')
                        <span class="text-danger text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    ...
                    <input id="expired_date" type="date" name="expired_date" value="{{ old('expired_date') }}" class="form-input" required/>
                    @error('expired_date')
                        <span class="text-danger text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    ...
                    <textarea id="keterangan" name="keterangan" rows="3" class="form-input" placeholder="Masukkan keterangan (opsional)">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <span class="text-danger text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <button type="submit" class="btn btn-primary !mt-6">Simpan Transaksi</button>
        </form>
